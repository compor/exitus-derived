...
if [ -z ${INPUT_FILE:+x} ]; then
  echo "error: input file name was not provided" > $ERRS
...
info: benchmark root dir: ${BMK_SOURCE_DIR}
...
for BMK in ${BENCHMARKS[@]}; do
  ...
  pushd ${BMK_SOURCE_DIR}/${BMK} > $OUTS

  if [ -e "${INPUT_FILE}" ]; then
    awk '{ if($1 ~ /^#/) print }' ${INPUT_FILE} | sed 's/#//' > ${OUTFILENAME};
  fi
